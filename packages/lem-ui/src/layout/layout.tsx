...
const Basic = (props: BasicPropsWithTagName) => {
  ...
  return React.createElement(tagName, { className: classNameString, ...resetProps }, children);
};

const BasicLayout: React.FC<BasicPropsWithTagName> = (props) => {
  const { prefixCls, className, tagName: Tag, isVertical, children, ...resetProps } = props;
  const classNameString = classNames(prefixCls, className, {
